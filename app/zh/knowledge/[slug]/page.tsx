...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!article) return {};
  const local = localizedArticle(article, "zh");
  return {
    ...
    openGraph: {
      title: local.seoTitle,
      description: local.metaDescription,
      url: absoluteUrl(`/zh/knowledge/${slug}`),
    ...
    alternates: {
      canonical: absoluteUrl(`/zh/knowledge/${slug}`),
      languages: {
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const article = getKnowledgeArticle(slug);
  if (!article) notFound();
  return <KnowledgeArticleView article={article} lang="zh" />;
}
