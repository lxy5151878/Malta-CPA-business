...
export default function TaxServicesPage({ lang }: { lang: Lang }) {
  const copy = getSiteCopy(lang);
  const p = copy.pages.taxServices;
        ...
        <div className="row g-4 g-lg-5 align-items-stretch">
          <div className="col-lg-7">
            <div className="bg-secondary rounded-1 h-100 p-4 p-lg-5">
              <h2 className="h1 mb-3">{p.advisory.title}</h2>
              <p className="fs-lg mb-3">{p.advisory.p1}</p>
              <p className="mb-3">{p.advisory.p2}</p>
              <p className="mb-0">{p.advisory.p3}</p>
            </div>
          </div>

          <div className="col-lg-5">
            <div className="bg-secondary rounded-1 h-100 p-4 p-lg-5">
              <h3 className="h2 mb-3">{p.advisory.listTitle}</h3>
              <ul className="list-unstyled mb-0">
                {p.advisory.items.map((t) => (
                  <li className="d-flex pt-1 mt-2" key={t}>
      ...
      <section className="container pb-5 mb-md-3 mb-lg-4 mb-xxl-5">
        ...
        <div className="row justify-content-center">
          <div className="col-lg-10 col-xl-9">
            <div className="accordion" id="taxFaq">
              {p.faq.items.map((f, idx) => {
                const headingId = `taxFaqH-${idx}`;
                const collapseId = `taxFaqC-${idx}`;
                const open = idx === 0;
