...
export default function PayrollPageView({ lang }: { lang: Lang }) {
  const copy = getSiteCopy(lang);
  const p = copy.pages.payroll;
  ...
  return (
    <main className="page-wrapper">
              ...
              <div className="card-body p-4 p-lg-5">
                ...
                <div className="d-flex gap-3 align-items-start">
                  <div className="btn btn-icon btn-lg btn-outline-primary rounded-circle pe-none flex-shrink-0">
                    <i className={p.header.callout.icon} />
                  </div>
                  <div>
      ...
      {/* Compliance + Why outsource */}
      <section className="container pb-5 mb-md-3 mb-lg-4 mb-xxl-5">
        <div className="row align-items-center g-4 g-lg-5">
          <div className="col-lg-6">
            <div className="card border-0 bg-secondary rounded-5 h-100">
              <div className="card-body p-4 p-lg-5">
                <h2 className="h1 mb-3">{p.compliance.left.title}</h2>
                <p className="text-body-secondary mb-4">{p.compliance.left.body}</p>

                <h3 className="h5 mb-3">{p.compliance.left.listTitle}</h3>
                <ul className="list-unstyled mb-0">
                  {p.compliance.left.items.map((n) => (
                    <li className="d-flex align-items-start mb-2" key={n}>
          ...
          <div className="col-lg-6">
            <div className="ps-lg-2">
              <h2 className="h1 mb-3">{p.compliance.right.title}</h2>
              <p className="fs-lg text-body-secondary mb-4">{p.compliance.right.body}</p>

              <div className="pt-2">
                <a className="btn btn-primary btn-lg" href={p.compliance.right.ctaHref}>
                  {p.compliance.right.ctaLabel}
                </a>
              </div>
            </div>
          </div>
          ...
          <div className="col-lg-5">
            <div className="card border-0 bg-secondary rounded-5 h-100">
              <div className="card-body p-4 p-lg-5">
                ...
                <ul className="list-unstyled mb-0">
                  {p.resources.items
                    .filter((r) => !/saving|excel|checkbook|balancer|grid|wallet/i.test(`${r.title} ${r.icon}`))
                    .map((r) => {
                      const isVatTool = /vat/i.test(r.title);
                      const href = "href" in r
                        ? r.href
                        : isVatTool
                          ? "#vat-calculator"
                          : "https://irdpubserv.gov.mt/services/ssccalculator.aspx";
                      const title = "href" in r ? r.title : isVatTool ? "VAT Calculator" : "MTCA SSC Calculator";
                      const note = "href" in r ? r.note : isVatTool ? "Use now" : "Official tool";

                      return (
                    <li className="py-2" key={r.title}>
                      <a
                      ...
                      <span className="badge bg-light text-body-secondary border">{note}</span>
                      </a>
                    </li>
                      );
                    })}
      ...
      {/* FAQ */}
      <section className="container pb-5 mb-lg-4 mb-xxl-5">
        <div className="row">
          ...
          <div className="col-lg-7">
            <div className="accordion" id="payrollFaq">
              {p.faq.items.map((f, idx) => {
                const id = `payrollFaq-${idx}`;
